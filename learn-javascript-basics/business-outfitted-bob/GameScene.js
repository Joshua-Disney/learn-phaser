...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Helper function to return an object containing evenly spaced x and y coordinates:function 

    function genRandomInt(coord) {
      let light = false
      let randomInt
      while (!light) {
        let temp = Math.floor(Math.random() * 5) * 75 + 25
        if (temp < coord - 25 || temp > coord + 25) {
          randomInt = temp
          light = true
        } else {
          light = false
    ...
    function generateRandomCoords() {
      ...
      return { x: genRandomInt(gameState.player.x), y: genRandomInt(gameState.player.y) }
